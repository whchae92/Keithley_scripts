...
def cycles(arr):  # Define a function that converts time in seconds to no of cycles
    arr_t = arr[:,0]
    for i in range(len(arr_t)):
        arr_t[i] = arr_t[i]/14.6 #choose length of a single cycle
    return arr_t


data_path1 = 'C:\\Users\\CHAE\\Dropbox (MIT)\\Research\\Keithley data\\mechanical\\050319\\050319_EPD_PEN_5000cycles_syringe_3ml.txt'
...
line, = ax1.plot(cycles(data1), data1[:,1], 'k')  # Plotting Time vs. Resistance
...
leg = plt.legend(['EPD-GO/AgNW'], prop=font)
